/* Chapter 4 · Step 3 · Voice Guide Agent harness
 *
 * INVARIANTS:
 *   1. A Starter-tier founder with a delivered Phase 01 foundation
 *      dispatches voice_guide_agent through production /api/agents/run
...
const WAR_TABLE_CONTENT = {
  schema_version: '1.0',
  header: { eyebrow: '01 Discovery · War Table', title: 'The Steadfield War Table', agent: 'war_table_synthesizer', generated_at: new Date().toISOString(), version: 1 },
  body_sections: [
    { heading: 'The audience', prose: 'Founders burnt out on growth hacking. They want compounding, not spikes.\n\nThey speak in seasons too; they just forgot.' },
  ],
  data_blocks: [
    { type: 'priority_list', title: 'Initiatives', content: { items: [
      { rank: 1, label: 'Own the patient-growth position', rationale: 'Nobody else will wait.' } ] } },
  ],
...
async function makeUser(tier) {
  const email = `qb-ch4s3-${uuid().slice(0, 8)}@qb-harness.test`;
...
async function main() {
  ...
  const out = { harness: 'voice-guide-agent', started_at: startedAt, base_url: BASE };
  ...
  try {
    ...
    await insertDelivered(starter.id, 'war_table_synthesizer', WAR_TABLE_CONTENT, 1);
    const happy = [];
    for (let i = 1; i <= HAPPY_RUNS; i++) {
      ...
      happy.push({
        ...
        rule_groups: row?.[0]?.content?.data_blocks?.[0]?.content?.groups?.length ?? 0,
      });
      console.error(`[happy ${i}] http=${res.http} delivered=${happy[i - 1].delivered} duration_ms=${happy[i - 1].duration_ms} rule_groups=${happy[i - 1].rule_groups}`);
    ...
    await insertDelivered(free.id, 'war_table_synthesizer', WAR_TABLE_CONTENT, 1);
  ...
  const happyOk = (out.happy_runs || []).length === HAPPY_RUNS && out.happy_runs.every(r => r.delivered && r.rule_groups === 3);
  ...
  fs.writeFileSync('tests/chapter-04/voice-guide-agent.last-run.json', JSON.stringify(out, null, 2));
