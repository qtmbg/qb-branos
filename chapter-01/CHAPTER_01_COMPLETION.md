...
- **Lock endpoint 504 on contention. UX lies to the user.** When the synchronous-await lock exceeds Vercel Edge's ~25 s budget, the endpoint returns HTTP 504. The browser toast reads "Lock failed: HTTP 504." But the lock and dispatch BOTH succeed server-side: `foundation_locked_at` is written, all four agents are dispatched, all four artifacts deliver, all five emails fire. The user doesn't know. A page refresh reveals the locked state correctly. This is a UX lie at the conversion moment and is Chapter 2's first priority.
- **PR #59 stuck dispatch root cause is a hypothesis, not confirmed.** Async dispatch shipped in PR #59, reverted in PR #63 after 1 of 10 verification runs surfaced 0 artifacts at 60 s (Run 5, dispatch_id `7cffe100-afb2-4a46-960b-4e79db2b5b05`). The working hypothesis is that fire-and-forget `fetch()` from a parent Edge function is cancelled when the parent returns before the child request establishes its own function invocation. This is consistent with Vercel runtime behavior but was not instrumented end-to-end during the 10-run test. Chapter 2 should reproduce in a controlled environment (Vercel logs filtered by request id, child function entry logging) before deciding the framework architecture. Until reproduced, treat root cause as unconfirmed.
- **Migration 010 (`dispatch_jobs` table) is already on prod for the revival.** Schema is in place; only the lock-foundation/dispatch code was reverted. Chapter 2 wires fresh code against the same table.
- **Alternative paths for Chapter 2 to consider:** durable job runner (Vercel Queues, Inngest, Trigger.dev), or pre-inserted artifact rows + reliable background dispatch (still Edge-based but with the artifact rows surviving cancellation so the polling client sees state).
