...
- **Visuals are programmatic.** Generated visuals (illustrations, hero art, mood references) are produced as **CSS/SVG/Claude-authored SVG**, not as raster images from a third-party image-gen API. The platform's value is rendering the brand identity *in the user's own system* — stock AI photos dilute that. SVG is also resolution-independent, themeable by injecting the user's palette hex codes, and prints cleanly into the `*-pdf.html` variants. If a tool genuinely needs photo-realism in the future, add a single-purpose direct-to-Google `api/gemini.js` proxy — never an aggregator.
