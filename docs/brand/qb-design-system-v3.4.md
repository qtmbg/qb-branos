...
- Updated Part 16 to remove the "no logo file" exclusion that v3.3 carried over from older drafts.
- Updated Master Instruction sync block (Part 14) to reference the new brand mark chapter.
...
The dot before the tag content is a brand-system motif. It is rose by default. Phase tags swap the dot color to the phase token (Phase 02 tag has periwinkle dot, Phase 04 has peach dot, etc.). The dot is **not the brand mark** — the brand mark is the QB monogram defined in Part 21.
...
## Part 21 — Brand Mark & Wordmark *(NEW in v3.4)*

The QB visual identity is anchored by a single mark and a single wordmark. Both are locked. This chapter is the source of truth — the standalone qb-logo-system.html file in the repo is a visual reference for this spec, not a competing source.

### 21.1 Concept

The mark reads as **infinity at a glance, Q+B on closer inspection**.

Two lobes meeting at a central crossing:
- **Left lobe** — round (Q), enclosed loop.
- **Right lobe** — articulated with a central pinch (B), a figure-eight on its side.
- **Central crossing** — the point where the two lobes meet, the X-axis of the system.
- **Q tail** — a short descending stroke that clarifies the Q. Without it, the mark reads as generic infinity. The tail is non-negotiable.

The mark is **three strokes** in execution but **one continuous reading** in perception. Hand-drawn editorial character, not a parametric symbol.

### 21.2 Construction (the locked SVG)

The mark is delivered as inline SVG using `currentColor` so it inherits the parent text color. Three paths, no fills, no gradients, no shadows on the mark itself.

**Canonical SVG (viewBox 0 0 280 130):**
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 130" fill="none"
     ...
     aria-label="Quantum Branding" role="img">
  <!-- Q lobe (left, round) -->
  <path d="M 138,60 C 138,22 92,18 62,30 C 26,46 26,76 62,90 C 92,102 138,98 138,60 Z"/>
  <!-- B lobe (right, with central pinch) -->
  <path d="M 138,60 C 138,22 184,20 214,30 C 248,42 248,55 214,60 C 248,65 248,80 214,92 C 184,102 138,98 138,60 Z"/>
  <!-- Q tail (descending stroke) -->
...
```
viewBox: 0 0 280 130
stroke-width: 10 (default), 11–14 in app icon contexts
stroke-linecap: round
stroke-linejoin: round
fill: none
```

Treat the stroke width as part of the design. Do not scale stroke-width independently when resizing the SVG. Use `vector-effect="non-scaling-stroke"` only if a specific render context demands it.

### 21.3 Anatomy (the four parts)

Every reference to the mark uses these names:

1. **Q lobe** — the left round enclosed loop.
2. **B lobe** — the right articulated loop with central pinch.
3. **Crossing** — the central point where the lobes meet at coordinate `(138, 60)`.
4. **Q tail** — the descending stroke from `(108, 92)` to `(128, 118)`. The tail is what makes the Q a Q. Removing it turns the mark into a generic infinity loop.

### 21.4 Four permitted colorways

The mark renders in **exactly four colorways**. Nothing else.

| Colorway | Use | Stroke color | Background |
|----------|-----|--------------|------------|
| **Ink** (default) | Body of work, navigation, footer, business cards | `var(--ink)` `#2D1521` | `var(--cream)` `#FBF5E6` |
| **Gold** | Premium tier moments, editorial features, founder block | `var(--gold)` `#E0B069` | `var(--cream)` or `var(--cream-card)` |
| **Rose** | Emotive contexts, intimate user-facing moments, brand love letters | `var(--rose)` `#CA6180` | `var(--cream)` or `var(--cream-rose)` |
| **Reverse** | Dark-section logos only (footer-on-ink, hero overlays) | `var(--cream)` `#FBF5E6` | `var(--ink)` `#2D1521` |

**Rules:**
- Ink is the default. If you can't justify another colorway in one sentence, use ink.
- Gold and rose never carry the mark on backgrounds lighter than the mark's stroke. Contrast is non-negotiable.
- Reverse never appears on any background lighter than `--ink`. Period.
- No gradients on the mark. No shadows on the mark itself (the cards holding the mark may carry the system's hard offset shadow, but the mark itself stays flat).
- No phase colors as mark colors. Phase tokens are for tags, dots, illustration accents — not the mark.
- No outlines on the mark. The strokes are the design.

### 21.5 Wordmark

The wordmark is **"quantum branding"** set in **Fraunces, italic, weight 600, SOFT 60, opsz 80**.
...
**When to use which:**
- **Italic (default)** — primary lockups, hero headers, footer, marketing pages, any moment of brand expression.
- **Roman** — utility contexts where italic creates legibility issues (very small sizes, dense UI, app sidebar headers, system text).

The wordmark is always lowercase. Capitalising "Quantum Branding" in running prose is fine for SEO, body copy, and meta tags. The wordmark glyph itself is always lowercase italic Fraunces.

### 21.6 Lockups (mark + wordmark)

Two locked combinations. Use one or the other. Do not invent third arrangements.

#### Horizontal lockup
For headers, footers, business cards, any horizontal real estate.

```html
<div class="qb-lockup qb-lockup--h">
  <svg class="qb-lockup_mark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 130"
  ...
  <div class="qb-lockup_text">
    <span class="qb-wordmark">quantum branding</span>
    <span class="qb-lockup_tagline">From idea to orbit</span>
  </div>
</div>
```

```css
.qb-lockup--h { display: inline-flex; align-items: center; gap: var(--space-s); color: var(--ink); }
.qb-lockup--h .qb-lockup_mark { width: clamp(44px, 6vw, 64px); height: auto; flex: 0 0 auto; }
.qb-lockup--h .qb-lockup_text { display: flex; flex-direction: column; line-height: 1.1; }
.qb-lockup--h .qb-wordmark { font-size: var(--step-1); }
.qb-lockup--h .qb-lockup_tagline {
  font-family: var(--font-mono); font-size: var(--step--2); font-weight: 500;
  letter-spacing: 0.12em; text-transform: uppercase; color: var(--ink-50);
}
```

The tagline ("From idea to orbit") is optional. Use it on home page header, business cards, and pitch contexts. Drop it everywhere else — repetition burns it out.

#### Vertical lockup
For app launchers, square crops, social profile images, splash screens.

```html
<div class="qb-lockup qb-lockup--v">
  <svg class="qb-lockup_mark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 130"
       fill="none" stroke="currentColor" stroke-width="10"
       stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    <path d="M 138,60 C 138,22 92,18 62,30 C 26,46 26,76 62,90 C 92,102 138,98 138,60 Z"/>
...
```css
.qb-lockup--v {
  display: inline-flex; flex-direction: column; align-items: center; gap: var(--space-2xs);
  color: var(--ink); text-align: center;
}
.qb-lockup--v .qb-lockup_mark { width: clamp(80px, 12vw, 120px); height: auto; }
.qb-lockup--v .qb-wordmark { font-size: var(--step-2); }
.qb-lockup--v .qb-lockup_label {
  font-family: var(--font-mono); font-size: var(--step--2); font-weight: 500;
...
| Context | Mark | Wordmark | Tagline | Notes |
|---------|------|----------|---------|-------|
| Browser tab favicon | Yes (32×32, ink on cream) | No | No | Mark only. Single SVG path bundle, served as `.svg` favicon with PNG fallback. |
| App icon (iOS/Android) | Yes (1024×1024 source, stroke-width 14) | No | No | Mark only, ink on cream background, rounded square handled by OS. |
| Marketing nav (cream pill) | Optional | Yes (italic, ink) | No | Wordmark with the rose dot brand mark before it. Mark is optional in the nav pill. |
| Marketing footer | Yes (small, 24–32px) | Yes (italic, ink) | Yes | Full horizontal lockup, optional tagline. |
| Hub navigation (signed-in) | Yes (small, 28px) | Yes (italic) | No | Sticky nav uses horizontal lockup without tagline. |
| Tool headers (agents) | No | Yes (small italic) | No | Wordmark only as a light brand presence. The tool's own header dominates. |
| Hero / brand moments | Optional | Optional | No | Most marketing heroes carry the headline, not the lockup. |
| Founder block | No | No | No | The founder portrait and the founder name carry the section. The lockup would crowd it. |
| Email signatures | Yes (24px) | Yes (italic, ink) | Yes | Full horizontal lockup. |
| Business cards | Yes (large, 32mm) | Yes (italic, ink) | Yes | Full horizontal lockup, gold variant for premium. |
| Social profile images | Yes (vertical lockup, 1:1 crop) | Yes | No | Mark + wordmark, no tagline. Rose variant on cream-rose background works well. |
| Splash screens (loading) | Yes (large, animated reveal) | Yes (italic) | No | Vertical lockup. The mark animates with a single 0.8s draw using `--ease-qb`. |

### 21.8 Clear space and minimum size

**Clear space:** the height of the Q lobe on all sides minimum. Roughly `0.6×` the mark's bounding-box height. No type, illustration, or interactive element inside that envelope.

**Minimum sizes:**
- Mark alone: 24px wide. Below this, the Q tail drops out and the mark reads as generic infinity. Don't go smaller.
- Horizontal lockup: 44px tall. Below this, the wordmark hits sub-12px and breaks legibility.
- Vertical lockup: 80px wide. Below this, the label line becomes unreadable.

For favicons (16–32px context), use the mark alone with `stroke-width: 12` instead of the default 10. The chunkier stroke holds at small sizes.

### 21.9 Animation patterns

The mark animates only at three moments. Outside these, it stays still.

**1. Mark reveal (page load):**
On first paint of marketing pages, the mark draws itself in with a 0.8s stroke-dasharray reveal. Three paths animate simultaneously, each with a slight delay.

```css
.qb-mark-reveal path {
  stroke-dasharray: 600;
  stroke-dashoffset: 600;
  animation: qb-mark-draw 0.8s var(--ease-qb) 0.2s forwards;
}
.qb-mark-reveal path:nth-child(2) { animation-delay: 0.35s; }
.qb-mark-reveal path:nth-child(3) { animation-delay: 0.55s; }
@keyframes qb-mark-draw { to { stroke-dashoffset: 0; } }

@media (prefers-reduced-motion: reduce) {
  .qb-mark-reveal path { stroke-dashoffset: 0; animation: none; }
}
```
...
**3. Hover (interactive lockup only):**
On hover, the lockup's mark scales 1.04 with a 0.4s spring. Wordmark stays still.

```css
.qb-lockup--interactive .qb-lockup_mark { transition: transform 0.4s var(--ease-qb); }
.qb-lockup--interactive:hover .qb-lockup_mark { transform: scale(1.04); }
```

The mark never spins, flashes, color-shifts on hover, or carries decorative pulses outside these three patterns.

### 21.10 File deliverables

The repo contains:

```
/img/brand/
  mark-ink.svg              — default, ink on transparent
  mark-gold.svg             — gold variant
  mark-rose.svg             — rose variant
  mark-reverse.svg          — cream variant for dark backgrounds
  mark-favicon.svg          — favicon-optimized (stroke 12, 32×32 viewBox)
  mark-favicon-32.png       — fallback favicon
  mark-app-icon-1024.png    — app icon source
  lockup-h-ink.svg          — horizontal lockup, ink
  lockup-h-gold.svg         — horizontal lockup, gold
  lockup-v-ink.svg          — vertical lockup, ink
  lockup-v-gold.svg         — vertical lockup, gold
  qb-logo-system.html       — visual reference page (read-only canon)
```

Inline SVG with `currentColor` is preferred over `<img>` references in HTML files because it lets the mark inherit text color, animate cleanly, and stay flat on file size.

### 21.11 Rules of use

**Do:**
- Use as inline SVG with `currentColor` so the mark inherits the parent's text color.
- Maintain clear space equal to the height of the Q lobe on all sides minimum.
- Pair the wordmark with Fraunces italic. Pair the tagline or label with JetBrains Mono uppercase.
- Use the mark alone for favicons, app icons, and any context where the wordmark would render below 14px.
- Animate the mark only with the three patterns in 21.9.

**Don't:**
- Recolor outside the four permitted colorways. No gradients, no shadows on the mark itself.
- Stretch, skew, rotate, or apply non-uniform scaling. Stroke-width is part of the design.
- Drop the Q tail to "simplify" the mark. The tail is what makes Q a Q.
- Apply the reverse cream variant on any background lighter than `--ink`.
- Set the wordmark in any font other than Fraunces. Inter is not an acceptable substitute.
- Use the mark as a background pattern, watermark, or repeated decorative element.
- Set the wordmark in title case ("Quantum Branding" with capital letters in the glyph). Lowercase italic only.
- Create new lockup arrangements. Two locked: horizontal and vertical. That's it.
- Generate new colorway variants for "this campaign" or "this client." White-label client delivery uses the existing four colorways with the client's accent color expressed elsewhere on the page.

### 21.12 Litmus test (mark-specific)

Before shipping any logo placement, ask:
1. Is this one of the four permitted colorways?
2. Does the Q tail render? (If not, the mark is too small or wrong path.)
3. Is the wordmark Fraunces italic 600 lowercase?
4. Is the clear space at least the height of the Q lobe?
5. Does the placement match a row in the Part 21.7 in-context table?

Five yeses. Otherwise, stop.
